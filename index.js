...
app.get('/api/persons/:id', (req, res, next) => {
  ...
  ContactModel.findById(id)
    .then(contact => {
      if (contact) { res.json(contact) } else { res.status(404).end() }
    })
    .catch(err => next(err))
})
...
app.put('/api/persons/:id', (req, res, next) => {
  ...
  if (body.name === undefined || body.number === undefined) { return res.status(400).json({ error: 'Name or number is missing' }) }
...
app.delete('/api/persons/:id', (req, res, next) => {
  ...
  ContactModel.findByIdAndDelete(id)
    .then(contact => {
      if (contact) { res.status(204).end() } else { res.status(404).end() }
    })
    .catch(err => next(err))
})
